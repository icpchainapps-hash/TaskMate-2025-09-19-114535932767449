...
import { useGetFeedPostComments, useAddFeedPostComment, useGetCallerUserProfile, useGetUserProfiles } from '../hooks/useQueries';
import { useInternetIdentity } from '../hooks/useInternetIdentity';
import { Principal } from '@dfinity/principal';
import type { FeedPostComment } from '../hooks/useQueries';
...
export default function FeedPostComments({ postId, compact = false, onToggleExpand }: FeedPostCommentsProps) {
  const { identity } = useInternetIdentity();
  const { data: userProfile } = useGetCallerUserProfile();
  const { data: comments = [], isLoading, error } = useGetFeedPostComments(postId);
  const addComment = useAddFeedPostComment();
    ...
    return (
      <div className="space-y-3">
        ...
        {identity && userProfile && (
          <form onSubmit={handleSubmitComment} className="space-y-2" onClick={(e) => e.stopPropagation()}>
